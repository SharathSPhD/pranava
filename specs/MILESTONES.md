...
### E4 — Consolidated research report  ☑ DONE (2026-07-17)
- `PAPER.md`: a publishable working paper tying the whole arc together — the digital edition, the
  holism investigation *and its correction* (E2→E6→E7), the prosody gap (E5), the autoresearch loop
  (X0), the epistemic gate (X1), the NSM spec (X2), limitations, and reproducibility. The false-
  positive-and-correction is framed as the central methodological contribution.
- **Gate** (`gates/check.py E4`): report has an abstract + conclusion, reports the null AND the
  correction (incl. the hetvābhāsa verdict), cites the edition + experiments, and has a limitations
  section. Evidence: `PAPER.md`; gate_E4.json.
